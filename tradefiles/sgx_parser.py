...
def parse_io_xlsx(input_path, output_path):
    ...
    try:
        ...
        console.print(
            f"[bold red]Error:[/bold red] Input file not found at {input_path}"
        )
        return
    ...
    for sheet_name in workbook.sheetnames:
        ...
        for i, cell_value in enumerate(header):
            if (
                ...
                and cell_value != "Counterparty"
            ):
                contract_months[cell_value] = i

        if not contract_months:
            console.print(
                f"Skipping sheet '{sheet_name}' - no contract month columns found"
            )
            continue
    ...
    console.print(
        f"[bold blue]Total trades extracted from all sheets: {total_trades}[/bold blue]"
    )
