...
def create_reconciliation_dataframe(
    ...
    for i, match in enumerate(matches, 1):
        ...
        exchange_ids.extend(
            [t.internal_trade_id for t in match.additional_exchange_trades]
        )
    ...
    for i, trade in enumerate(
        unmatched_exchanges, len(matches) + len(unmatched_traders) + 1
    ):
        record = {
            "reconid": f"RECON_{i:06d}",
            "source_traders_id": [],
...
def display_reconciliation_dataframe(df: pd.DataFrame) -> None:
    ...
    for _, row in df.iterrows():  # Show ALL rows including unmatched
        # Format trader IDs
        trader_ids = (
            str(row["source_traders_id"])[:12] + "..."
            if len(str(row["source_traders_id"])) > 15
            else str(row["source_traders_id"])
        )
        trader_ids = trader_ids.replace("'", "").replace("[", "").replace("]", "")

        # Format exchange IDs
        exch_ids = (
            str(row["source_exch_id"])[:12] + "..."
            if len(str(row["source_exch_id"])) > 15
            else str(row["source_exch_id"])
        )
        ...
        # Format other fields
        confidence = (
            f"{row['confidence_score']:.1f}%" if row["confidence_score"] > 0 else "N/A"
        )
        quantity = f"{row['quantity']:,.0f}"
        price = f"{row['price']:,.2f}" if pd.notna(row["price"]) else "N/A"
        product = (
            str(row["product_name"])[:13] + "..."
            if len(str(row["product_name"])) > 15
            else str(row["product_name"])
        )
        remarks = (
            str(row["remarks"])[:13] + "..."
            if len(str(row["remarks"])) > 15
    ...
    summary_table.add_row(
        "✅ Matched",
        str(matched),
        f"{matched / total * 100:.1f}%" if total > 0 else "0.0%",
    )
    summary_table.add_row(
        "❌ Unmatched Traders",
        str(unmatched_traders),
        f"{unmatched_traders / total * 100:.1f}%" if total > 0 else "0.0%",
    )
    summary_table.add_row(
        "❌ Unmatched Exchange",
        str(unmatched_exch),
        f"{unmatched_exch / total * 100:.1f}%" if total > 0 else "0.0%",
    )
    if pending_exchange > 0:
        summary_table.add_row(
            "⏳ Pending Exchange",
            str(pending_exchange),
            f"{pending_exchange / total * 100:.1f}%",
        )
    if pending_approval > 0:
        summary_table.add_row(
            "📝 Pending Approval",
            str(pending_approval),
            f"{pending_approval / total * 100:.1f}%",
        )
