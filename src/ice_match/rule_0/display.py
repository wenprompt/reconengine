...
class PositionDisplay:
    ...
    def __init__(
        ...
        self.bbl_products = [
            p.lower()
            for p in (bbl_products if bbl_products is not None else ["brent swap"])
        ]
    ...
    def _display_product_matrix(
        ...
        for comp in comparisons:
            ...
            if is_bbl_product:
                ...
                trader_bbl = (
                    self._format_decimal(comp.trader_bbl)
                    if comp.trader_bbl is not None
                    else "N/A"
                )
                exchange_bbl = (
                    self._format_decimal(comp.exchange_bbl)
                    if comp.exchange_bbl is not None
                    else "N/A"
                )
                diff_bbl = (
                    self._format_decimal(comp.difference_bbl, show_sign=True)
                    if comp.difference_bbl is not None
                    else "N/A"
                )
                ...
                table.add_row(
                    comp.contract_month, trader_mt, exchange_mt, diff_display, status
                )
    ...
    def show_position_details(
        ...
        for product in sorted(all_products):
            ...
            all_pos_keys = (
                trader_matrix.positions.keys() | exchange_matrix.positions.keys()
            )
            ...
            if contract_month:
                months_with_product = {
                    m for m in months_with_product if m == contract_month
                }
            ...
            self.console.print(
                f"[bold cyan]TRADE DETAILS - {product} ({unit})[/bold cyan]"
            )
            self.console.print("=" * 80)
            self.console.print()
    ...
    def _show_month_details(
        ...
        exchange_details = sorted(
            exchange_details, key=get_absolute_quantity, reverse=True
        )
        ...
        trader_total_str = (
            f"Total: {self._format_decimal(trader_total, show_sign=True)}"
        )
        exchange_total_str = (
            f"Total: {self._format_decimal(exchange_total, show_sign=True)}"
        )

        table.add_row(
            f"[bold]{trader_total_str}[/bold]", f"[bold]{exchange_total_str}[/bold]"
        )
