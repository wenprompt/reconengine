...
class PositionComparison:
    ...
    def has_discrepancy(self) -> bool:
        ...
        return (
            ...
            and self.status != MatchStatus.ZERO_POSITION
        )
...
class MatrixComparator:
    ...
    def __init__(
        self, brent_conversion_ratio: Decimal, tolerance_mt: Decimal = Decimal("0.01")
    ):
        ...
        self.tolerance_bbl = (
            tolerance_mt * brent_conversion_ratio
        )  # BBL tolerance based on conversion ratio
    ...
    def _compare_position(
        ...
        trader_mt = (
            trader_pos.quantity_mt
            if trader_pos and trader_pos.quantity_mt is not None
            else Decimal("0")
        )
        trader_bbl = (
            trader_pos.quantity_bbl
            if trader_pos and trader_pos.quantity_bbl is not None
            else Decimal("0")
        )
        exchange_mt = (
            exchange_pos.quantity_mt
            if exchange_pos and exchange_pos.quantity_mt is not None
            else Decimal("0")
        )
        exchange_bbl = (
            exchange_pos.quantity_bbl
            if exchange_pos and exchange_pos.quantity_bbl is not None
            else Decimal("0")
        )

        # Calculate differences
        diff_mt = (
            trader_mt - exchange_mt if product.lower() != "brent swap" else Decimal("0")
        )
        diff_bbl = (
            trader_bbl - exchange_bbl
        ...
        if product.lower() == "brent swap":
            status = self._determine_status_bbl(trader_bbl, exchange_bbl, diff_bbl)
        else:
            status = self._determine_status_mt(trader_mt, exchange_mt, diff_mt)
    ...
    def _determine_status_mt(
        self, trader_mt: Decimal, exchange_mt: Decimal, diff_mt: Decimal
    ) -> MatchStatus:
    ...
    def _determine_status_bbl(
        self, trader_bbl: Decimal, exchange_bbl: Decimal, diff_bbl: Decimal
    ) -> MatchStatus:
