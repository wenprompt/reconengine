...
class AggregatedCrackMatcher(AggregationBaseMatcher):
    ...
    def find_matches(self, pool_manager: UnmatchedPoolManager) -> list[MatchResult]:
        ...
        logger.info(
            f"Processing {len(crack_trades)} crack trades for aggregated matching"
        )
        ...
        for crack_trade in crack_trades:
            ...
            for exchange_trade in exchange_trades:
                ...
                if (
                    exchange_trade.product_name.lower()
                    == crack_trade.product_name.lower()
            ...
            if not crack_candidates:
                logger.debug(
                    f"No crack product candidates found for {crack_trade.product_name}"
                )
                continue
    ...
    def _find_crack_aggregations_with_conversion(
        ...
        logger.debug(
            f"Crack aggregation: {len(many_groups)} groups from many_source, {len(one_source)} single trades"
        )
        ...
        for group_signature, group_trades in many_groups.items():
            ...
            total_quantity_bbl = Decimal(
                sum(trade.quantity_bbl for trade in group_trades)
            )
    ...
    def _validate_crack_aggregation_with_conversion(
        ...
        for trade in aggregated_trades[1:]:
            if not self._trades_have_matching_characteristics(
                ...
                logger.debug(
                    "Crack aggregation validation failed: aggregated trades have different characteristics"
                )
                return False
        ...
        if not self._trades_have_matching_characteristics(
            ...
            logger.debug(
                "Crack aggregation validation failed: aggregated and single trade characteristics don't match"
            )
            return False
        ...
        if not validate_mt_to_bbl_quantity_match(
            ...
            logger.debug(
                "Crack aggregation validation failed: MT→BBL quantity mismatch"
            )
            return False

        logger.debug(
            f"✅ Crack aggregation validation passed: {len(aggregated_trades)} trades convert to {single_trade.quantity_mt} MT"
        )
        return True
    ...
    def get_rule_info(self) -> dict[str, Any]:
        ...
        return {
            ...
            "tolerances": {"quantity_bbl": float(self.BBL_TOLERANCE)},
        }
