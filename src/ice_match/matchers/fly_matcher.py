...
class FlyMatcher(BaseMatcher):
    ...
    def find_matches(self, pool_manager: UnmatchedPoolManager) -> List[MatchResult]:
        ...
        exchange_fly_groups = self._find_exchange_fly_groups(exchange_trades, pool_manager)
        ...
        for trader_group in trader_fly_groups:
            ...
            match_result = self._match_fly_group(trader_group, exchange_fly_groups, pool_manager)
            if match_result:
                matches.append(match_result)
                
                # Record the match in the pool manager
                if not pool_manager.record_match(match_result):
                    logger.error("Failed to record fly match and remove trades from pool")
                else:
        ...
        for dealid_str, trades_in_group in dealid_groups.items():
            ...
            if len(trades_in_group) == 3:
                if self._is_exchange_fly_group(trades_in_group):
                    ...
                    logger.debug(f"Found dealid fly group: {[t.trade_id for t in trades_in_group]} (dealid: {dealid_str})")
                ...
                for i in range(len(trades_in_group)):
                    for j in range(i + 1, len(trades_in_group)):
                        for k in range(j + 1, len(trades_in_group)):
                            potential_group = [trades_in_group[i], trades_in_group[j], trades_in_group[k]]
                            if self._is_exchange_fly_group(potential_group):
                                fly_groups.append(potential_group)
                                logger.debug(f"Found dealid fly group: {[t.trade_id for t in potential_group]} (dealid: {dealid_str})")
    ...
    def _validate_fly_match(
        ...
        if len(trader_products) != 1 or len(exchange_products) != 1 or trader_products != exchange_products:
            return False
        ...
        return (self._validate_fly_directions(sorted_trader, sorted_exchange) and 
                self._validate_fly_price_calculation(sorted_trader, sorted_exchange))
    ...
    def _validate_fly_directions(
        ...
        return all(
            month in exchange_month_bs and trader_month_bs[month] == exchange_month_bs[month]
    ...
    def _validate_fly_price_calculation(
        ...
        exchange_fly_price = (exchange_x.price - exchange_y.price) + (exchange_z.price - exchange_y.price)
        
        logger.debug(f"Fly price validation: trader={trader_fly_price}, "
