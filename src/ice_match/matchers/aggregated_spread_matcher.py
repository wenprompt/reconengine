...
class AggregatedSpreadMatcher(MultiLegBaseMatcher):
    ...
    def find_matches(self, pool_manager: UnmatchedPoolManager) -> list[MatchResult]:
        ...
        for spread_pair in trader_spread_pairs:
            match = self._find_aggregated_spread_match(
            ...
            if match:
                ...
                if pool_manager.record_match(match):
                    ...
                    logger.error(
                        f"Failed to record aggregated spread match: {match.match_id}"
                    )
    ...
    def _is_valid_spread_pair(self, trade1: Trade, trade2: Trade) -> bool:
        ...
        return self.validate_spread_pair_characteristics(
            trade1, trade2, self.normalizer
        )
    ...
    def _find_aggregated_spread_match(
        ...
        price_contract_matches = aggregated_positions.get(
            price_trade.contract_month, []
        )
        zero_contract_matches = aggregated_positions.get(
            zero_price_trade.contract_month, []
        )
    ...
    def _aggregate_exchange_trades_by_contract(
        ...
        for trade in exchange_trades:
            ...
            if (
                ...
                or not self.validate_universal_fields(reference_trader_trade, trade)
            ):
                continue

            # Group by aggregation characteristics (contract, price, B/S)
            # Note: universal fields are already validated above
            group_key = (trade.contract_month, trade.price, trade.buy_sell)
    ...
    def _validate_aggregated_spread_match(
        ...
        if (
            price_ex_trades[0].buy_sell != price_trade.buy_sell
            or zero_ex_trades[0].buy_sell != zero_price_trade.buy_sell
        ):
