...
class AggregationBaseMatcher(BaseMatcher):
    ...
    def find_many_to_one_aggregations(
        ...
        logger.debug(f"Searching {len(many_groups)} groups against {len(one_index)} single trade signatures")
        ...
        for group_signature, group_trades in many_groups.items():
            ...
            logger.debug(f"Processing group with {len(group_trades)} trades: {[t.trade_id for t in group_trades]}")
            ...
            if match_signature in one_index:
                ...
                logger.debug(f"  Found {len(candidate_trades)} candidate trades: {[t.trade_id for t in candidate_trades]}")
                ...
                for candidate_trade in candidate_trades:
                    if not pool_manager.is_trade_matched(candidate_trade):
                        ...
                        if self.validate_aggregation_consistency(group_trades, candidate_trade):
                            aggregations.append((group_trades, candidate_trade))
                            logger.debug(f"✅ Found many-to-one aggregation: {len(group_trades)} trades → 1 trade ({candidate_trade.trade_id})")
                            break
                        else:
                            logger.debug(f"❌ Aggregation validation failed for {candidate_trade.trade_id}")
            else:
    ...
    def validate_aggregation_consistency(self, aggregated_trades: List[Trade], single_trade: Trade) -> bool:
        ...
        try:
            ...
            for trade in aggregated_trades[1:]:
                if not self._trades_have_matching_characteristics(first_trade, trade, ignore_quantity=True):
                    logger.debug("Aggregation validation failed: aggregated trades have different characteristics")
                    return False
            ...
            if not self._trades_have_matching_characteristics(first_trade, single_trade, ignore_quantity=True):
                logger.debug("Aggregation validation failed: aggregated and single trade characteristics don't match")
                return False
            ...
            if total_quantity != single_trade.quantity_mt:
                logger.debug(f"Aggregation validation failed: quantity sum {total_quantity} != {single_trade.quantity_mt}")
                return False
    ...
    def aggregate_trades_by_contract_and_characteristics(
        ...
        for trade in trades:
            ...
            if (trade.product_name != target_product or 
                not self.validate_universal_fields(reference_trade, trade)):
                continue
    ...
    def _create_full_trade_index(self, trades: List[Trade], base_fields: List[str]) -> Dict[tuple, List[Trade]]:
    ...
    def _trades_have_matching_characteristics(
        ...
        if (trade1.product_name != trade2.product_name or
            trade1.contract_month != trade2.contract_month or
            trade1.price != trade2.price or
    ...
    def create_aggregation_match_result(
        ...
        if direction == "trader_to_exchange":
            ...
            return MatchResult(
                ...
                additional_trader_trades=aggregated_trades[1:],  # Additional aggregated trader trades
            ...
            return MatchResult(
                ...
                additional_exchange_trades=aggregated_trades[1:],  # Additional aggregated exchange trades
                matched_fields=matched_fields,
                differing_fields=differing_fields,
                tolerances_applied={
