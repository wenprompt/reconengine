...
from ..utils.conversion_helpers import get_product_conversion_ratio
...
class ComplexCrackRollMatcher(MultiLegBaseMatcher):
    ...
    def find_matches(self, pool_manager: UnmatchedPoolManager) -> List[MatchResult]:
        ...
        logger.debug(f"Found {len(trader_crack_pairs)} potential trader crack roll pairs")
        ...
        for trader_pair in trader_crack_pairs:
            ...
            match_result = self._find_crack_roll_match(trader_pair, exchange_trades, pool_manager)
            if match_result:
                matches.append(match_result)
                
                # Record the match to remove trades from pool
                if not pool_manager.record_match(match_result):
                    logger.error("Failed to record complex crack roll match and remove trades from pool")
                else:
    ...
    def _find_consecutive_crack_pairs(self, trader_trades: List[Trade]) -> List[Tuple[Trade, Trade]]:
    ...
    def _validate_crack_roll_pattern(self, trade1: Trade, trade2: Trade) -> bool:
        ...
        if "crack" not in trade1.product_name.lower() or "crack" not in trade2.product_name.lower():
            return False
    ...
    def _find_crack_roll_match(
        self, trader_pair: Tuple[Trade, Trade], exchange_trades: List[Trade], pool_manager: UnmatchedPoolManager
    ...
    def _find_exchange_crack_position(
        ...
        for trade in exchange_trades:
            ...
            if (trade.contract_month == contract_month and 
                self.validate_universal_fields(trade, reference_trade)):
        ...
        if base_trade and brent_trade:
            ...
            if self._validate_crack_position_quantities(base_trade, brent_trade, reference_trade):
    ...
    def _validate_crack_position_quantities(
        ...
        product_ratio = get_product_conversion_ratio(reference_trade.product_name, self.config_manager)
    ...
    def _validate_crack_roll_match(
        self, trader_pair: Tuple[Trade, Trade], exchange_positions: Dict[str, Tuple[Trade, Trade]]
    ) -> bool:
        ...
        for month, (base_trade, brent_trade) in exchange_positions.items():
            ...
            crack_price = self._calculate_crack_price(base_trade, brent_trade, trader_for_month)
    ...
    def _calculate_crack_price(self, base_trade: Trade, brent_trade: Trade, trader_crack_trade: Trade) -> Optional[Decimal]:
    ...
    def _validate_crack_direction_logic(self, trader_crack: Trade, base_trade: Trade, brent_trade: Trade) -> bool:
    ...
    def _validate_roll_spread_calculation(
        ...
        logger.debug(f"Roll spread validation: calculated={roll_spread}, trader={trader_spread_price}")
        return True
