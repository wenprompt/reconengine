...
class MatchingConfig(BaseModel):
    ...
    bbl_to_mt_ratio: Decimal = Field(
        default=Decimal("6.35"),
        description="Conversion ratio from BBL to MT"
    )
    ...
    max_trades_per_batch: int = Field(
        default=10000,
        ge=1,
        description="Maximum trades to process in one batch"
    )
    ...
    show_unmatched: bool = Field(
        default=True,
        description="Whether to show unmatched trades in output"
    )
    
    show_statistics: bool = Field(
        default=True,
        description="Whether to show matching statistics"
    )
