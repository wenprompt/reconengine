...
class UnifiedJSONLoader:
    ...
    def load_json(self, json_path: Path) -> tuple[pd.DataFrame, pd.DataFrame]:
        ...
        try:
            ...
            logger.info(
                f"Loaded {len(trader_df)} trader records and {len(exchange_df)} exchange records"
            )
    ...
    def load_and_group_by_exchange(
        ...
        for group_id in all_groups:
            ...
            group_trader_df = (
                trader_df[trader_df["exchangegroupid"] == group_id]
                if not trader_df.empty
                else pd.DataFrame()
            )
            group_exchange_df = (
                exchange_df[exchange_df["exchangegroupid"] == group_id]
            ...
            if not group_trader_df.empty or not group_exchange_df.empty:
                ...
                logger.info(
                    f"Group {group_id}: {len(group_trader_df)} trader, {len(group_exchange_df)} exchange records"
                )
