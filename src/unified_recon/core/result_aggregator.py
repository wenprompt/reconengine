...
class ResultAggregator:
    ...
    def add_system_result(
        ...
        if match_rate is None:
            match_rate = (
                (matches_found / trader_count * 100) if trader_count > 0 else 0.0
            )
        ...
        logger.info(
            f"Added result for group {group_id} ({system_name}): {matches_found} matches, {match_rate:.1f}% rate"
        )
    ...
    def get_aggregated_results(self) -> UnifiedResult:
        ...
        if self.system_results:
            ...
            if total_trader_weight > 0:
                overall_match_rate = (
                    sum(r.match_rate * r.trader_count for r in self.system_results)
                    / total_trader_weight
                )
            else:
                overall_match_rate = 0.0
        ...
        for result in self.system_results:
            ...
            system_breakdown[result.system_name]["exchange_trades"] += (
        ...
        for system_name, stats in system_breakdown.items():
            system_results = [
                r for r in self.system_results if r.system_name == system_name
            ]
            if system_results:
                stats["avg_match_rate"] = sum(
                    r.match_rate for r in system_results
                ) / len(system_results)
        ...
        processing_summary = {
            ...
            "total_processing_time": sum(
                r.processing_time for r in self.system_results if r.processing_time
            ),
        }
