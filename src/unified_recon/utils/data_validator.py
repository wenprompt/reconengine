...
class DataValidator:
    ...
    def validate_csv_structure(self, df: pd.DataFrame, data_type: str, file_path: Path) -> bool:
        ...
        logger.info(f"Validated {data_type} CSV structure: {len(df)} rows, {len(df.columns)} columns")
        return True
    
    def validate_exchange_groups(self, df: pd.DataFrame, file_path: Path) -> Tuple[List[int], Dict[int, int]]:
        ...
        if null_groups > 0:
            logger.warning(f"Found {null_groups} rows with null exchangegroupid in {file_path}")
    ...
    def validate_data_consistency(self, trader_groups: List[int], exchange_groups: List[int]) -> bool:
        ...
        if trader_only:
            logger.warning(f"Exchange groups found in trader data but not exchange data: {trader_only}")
        
        # Check for groups in exchange data but not in trader data  
        exchange_only = exchange_set - trader_set
        if exchange_only:
            logger.warning(f"Exchange groups found in exchange data but not trader data: {exchange_only}")
        
        # Find common groups
        common_groups = trader_set & exchange_set
        if not common_groups:
            raise DataValidationError("No common exchange groups found between trader and exchange data")
        
        logger.info(f"Common exchange groups for processing: {sorted(common_groups)}")
        return True
    
    def validate_group_data_quality(self, df: pd.DataFrame, group_id: int, data_type: str) -> Dict[str, Any]:
        ...
        stats = {
            ...
            'null_prices': group_data['price'].isnull().sum() if 'price' in group_data.columns else 0,
            'zero_prices': (group_data['price'] == 0).sum() if 'price' in group_data.columns else 0,
            'unique_products': group_data['productname'].nunique() if 'productname' in group_data.columns else 0,
            'date_range': None
        }
