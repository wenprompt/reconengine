...
class SGXTrade(BaseModel):
    ...
    # Trading details
    product_name: str = Field(..., description="Normalized product name (FE, PMX, CAPE, etc.)")
    quantity_lots: Optional[Decimal] = Field(None, ge=0, description="Trade quantity in lots")
    ...
    spread: Optional[str] = Field(None, description="Spread indicator (S/B for spread legs)")
    ...
    trading_session: Optional[str] = Field(None, description="Trading session (T, T+1)")
    ...
    def matching_signature(self) -> str:
        ...
        return (
            f"{self.product_name}|{self.contract_month}|{self.quantity_units}|"
