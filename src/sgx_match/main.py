...
class SGXMatchingEngine:
    ...
    def __init__(self, config_manager: Optional[SGXConfigManager] = None):
        ...
        self.product_spread_matcher = ProductSpreadMatcher(
    ...
    def run_matching(
        ...
        try:
            ...
            trader_trades = self.trade_factory.from_csv(
                trader_csv_path, SGXTradeSource.TRADER
            )
            exchange_trades = self.trade_factory.from_csv(
                exchange_csv_path, SGXTradeSource.EXCHANGE
            )
            ...
            self.display.show_success(
                f"SGX matching completed. Found {len(all_matches)} total matches."
            )
            return all_matches
    ...
    def run_matching_from_dataframes(
        ...
        try:
            ...
            trader_trades = self.trade_factory.from_dataframe(
                trader_df, SGXTradeSource.TRADER
            )
            exchange_trades = self.trade_factory.from_dataframe(
                exchange_df, SGXTradeSource.EXCHANGE
            )
