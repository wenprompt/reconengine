...
class ProductSpreadMatcher(MultiLegBaseMatcher):
    ...
    def __init__(
        self, config_manager: SGXConfigManager, normalizer: SGXTradeNormalizer
    ):
    ...
    def find_matches(self, pool_manager: SGXUnmatchedPool) -> List[SGXMatchResult]:
        ...
        trader_product_spread_pairs = self._find_trader_product_spread_pairs(
            trader_trades, pool_manager
        )

        # Tier 1 & 2: Find exchange product spread pairs using dealid grouping
        exchange_product_spread_pairs = self._find_exchange_product_spread_pairs(
            exchange_trades, pool_manager
        )

        logger.debug(
            f"Found {len(trader_product_spread_pairs)} trader product spread pairs"
        )
        logger.debug(
            f"Found {len(exchange_product_spread_pairs)} exchange product spread pairs"
        )
        ...
        for trader_pair_with_tier in trader_product_spread_pairs:
            ...
            match_result = self._match_product_spread_pair(
                trader_trades_list,
                exchange_product_spread_pairs,
                pool_manager,
                confidence_tier,
            )
            if match_result:
                ...
                if pool_manager.record_match(match_result):
                    ...
                    logger.debug(
                        f"Created product spread match: {match_result.match_id}"
                    )
                else:
                    logger.warning(
                        f"Failed to atomically record product spread match {match_result.match_id}"
                    )

        # Tier 3: Process hyphenated exchange spreads vs trader pairs (1-to-2)
        hyphenated_matches = self._find_hyphenated_exchange_matches(
    ...
    def _find_trader_product_spread_pairs(
        ...
        ps_trades = [
            ...
            if trade.spread and "PS" in str(trade.spread).upper()
        ]
        logger.debug(f"Total PS trades in trader_trades: {len(ps_trades)}")
        for trade in ps_trades:
            logger.debug(
                f"PS trade: {trade.product_name}/{trade.buy_sell}, price={trade.price}, contract_month={trade.contract_month}, unmatched={pool_manager.is_unmatched(trade.internal_trade_id, SGXTradeSource.TRADER)}"
            )
        ...
        for trades in trade_groups.values():
            ...
            if len(trades) >= 1:
                ...
                for idx, trade in enumerate(trades):
                    logger.debug(
                        f"  Trade {idx}: {trade.product_name}/{trade.buy_sell}, price={trade.price}, spread={trade.spread}, contract_month={trade.contract_month}, quantity={trade.quantityunit}"
                    )
            ...
            if len(trades) >= 2:
                for i in range(len(trades)):
                    for j in range(i + 1, len(trades)):
                        logger.debug(
                            f"Checking trader pair: {trades[i].product_name}/{trades[i].buy_sell} + {trades[j].product_name}/{trades[j].buy_sell}"
                        )
                        is_match, confidence_tier = self._is_trader_product_spread_pair(
                            trades[i], trades[j]
                        )
                        if is_match:
                            tier_desc = (
                            ...
                            logger.debug(
                                f"Found trader product spread pair (Tier {confidence_tier} - {tier_desc}): {trades[i].internal_trade_id} + {trades[j].internal_trade_id}"
                            )
                            # Store trades with confidence tier information
                            product_spread_pairs.append(
                                (trades[i], trades[j], confidence_tier)
                            )
    ...
    def _is_trader_product_spread_pair(
        ...
        if (
            ...
            or trade1.product_name == trade2.product_name
        ):
            return False, 0
        ...
        has_ps_indicator = (trade1.spread and "PS" in str(trade1.spread).upper()) and (
        ...
        has_identical_spread_price = (
            trade1.price != 0 and trade2.price != 0 and trade1.price == trade2.price
        )
    ...
    def _find_exchange_product_spread_pairs(
        ...
        for trade in exchange_trades:
            ...
            if (
                dealid is not None
                and tradeid
                and str(dealid).strip()
                and str(tradeid).strip()
            ):
            ...
            if len(trades_in_group) == 2:
                ...
                if tradeid1 != tradeid2 and tradeid1 and tradeid2:
                    ...
                    if self._validate_exchange_product_spread_pair(trade1, trade2):
                        ...
                        logger.debug(
                            f"Found dealid product spread pair: {tradeid1}/{tradeid2} (dealid: {dealid_str})"
                        )
                ...
                for i in range(len(trades_in_group)):
                    for j in range(i + 1, len(trades_in_group)):
                        ...
                        if tradeid1 != tradeid2 and tradeid1 and tradeid2:
                            if self._validate_exchange_product_spread_pair(
                                trade1, trade2
                            ):
                                product_spread_pairs.append([trade1, trade2])
                                logger.debug(
    ...
    def _validate_exchange_product_spread_pair(
        self, trade1: SGXTrade, trade2: SGXTrade
    ) -> bool:
    ...
    def _match_product_spread_pair(
        ...
        for exchange_pair in exchange_product_spread_pairs:
            ...
            if self._validate_product_spread_match(trader_pair, exchange_pair):
                return self._create_product_spread_match_result(
                    trader_pair, exchange_pair, confidence_tier
                )
    ...
    def _validate_product_spread_match(
        ...
        if not self.validate_options_compatibility(
            trader_trade1, trader_trade2, exchange_trade1, exchange_trade2
        ):
        ...
        if not self._validate_exchange_product_spread_pair(
            exchange_trade1, exchange_trade2
        ):
            logger.debug(
                "Product spread validation failed: invalid exchange product spread pair"
            )
            return False
        ...
        if trader_products != exchange_products:
            logger.debug(
                f"Product spread validation failed: product mismatch - trader: {trader_products}, exchange: {exchange_products}"
            )
            return False

        # Validate contract months match
        if (
            trader_trade1.contract_month != exchange_trade1.contract_month
            or trader_trade1.contract_month != exchange_trade2.contract_month
        ):
            logger.debug(
                f"Product spread validation failed: contract month mismatch - trader: {trader_trade1.contract_month}, exchange: {exchange_trade1.contract_month}/{exchange_trade2.contract_month}"
            )
            return False

        # Validate quantities match
        if (
            trader_trade1.quantityunit != exchange_trade1.quantityunit
            or trader_trade1.quantityunit != exchange_trade2.quantityunit
        ):
            logger.debug(
                f"Product spread validation failed: quantity mismatch - trader: {trader_trade1.quantityunit}, exchange: {exchange_trade1.quantityunit}/{exchange_trade2.quantityunit}"
            )
            return False

        # Validate B/S directions and product spread prices
        directions_valid = self._validate_product_spread_directions(
            trader_trades, exchange_trades
        )
        prices_valid = self._validate_product_spread_prices(
            trader_trades, exchange_trades
        )
    ...
    def _validate_product_spread_directions(
        ...
        if (
            trader_directions != expected_directions
            or exchange_directions != expected_directions
        ):
            return False
    ...
    def _calculate_tier_confidence(self, confidence_tier: int) -> Decimal:
        ...
        if confidence_tier in tier_config:
            ...
            logger.debug(
                f"Tier {confidence_tier} ({description}): {base_confidence} - {adjustment} = {tier_confidence}%"
            )
        else:
            # Default to base confidence for unknown tiers
            tier_confidence = base_confidence
            logger.warning(
                f"Unknown confidence tier {confidence_tier}, using base confidence {base_confidence}%"
            )
    ...
    def _find_hyphenated_exchange_matches(
        ...
        hyphenated_trades = [
            ...
            if pool_manager.is_unmatched(t.internal_trade_id, SGXTradeSource.EXCHANGE)
            and "-" in t.product_name
        ...
        logger.debug(
            f"Processing {len(hyphenated_trades)} hyphenated exchange products for Tier 3"
        )

        # Create index of trader trades by signature for efficient lookup
        trader_index = self._create_trader_index_for_hyphenated(
            trader_trades, pool_manager
        )

        for exchange_trade in hyphenated_trades:
            match = self._find_hyphenated_product_spread_match(
            ...
            if match:
                ...
                if pool_manager.record_match(match):
                    ...
                    logger.debug(
                        f"Found Tier 3 hyphenated product spread match: {match.match_id}"
                    )
                else:
                    logger.warning(
                        f"Failed to atomically record Tier 3 match {match.match_id}"
                    )
    ...
    def _create_trader_index_for_hyphenated(
        ...
        logger.debug(
            f"Created trader index for hyphenated matching with {len(index)} signatures"
        )
        return index
    ...
    def _find_hyphenated_product_spread_match(
        ...
        for trade in matching_trades:
            if pool_manager.is_unmatched(
                ...
                if trade.product_name == first_product:
                    first_trade = trade
                    logger.debug(
                        f"Found first product match: {trade.internal_trade_id}"
                    )
                elif trade.product_name == second_product:
                    second_trade = trade
                    logger.debug(
                        f"Found second product match: {trade.internal_trade_id}"
                    )
        ...
        if not self._validate_hyphenated_spread_match(
        ...
        return self._create_product_spread_match_result(
            [first_trade, second_trade], [exchange_trade], confidence_tier=3
        )
    ...
    def _validate_hyphenated_spread_match(
        ...
        try:
            ...
            if not self.validate_options_compatibility(
                ...
                logger.debug(
                    "❌ Hyphenated spread validation failed: options/futures mismatch"
                )
                return False
            ...
            if first_trader_trade.buy_sell == second_trader_trade.buy_sell:
                logger.debug(
                    f"❌ Trader trades must have opposite B/S directions: {first_trader_trade.buy_sell}/{second_trader_trade.buy_sell}"
                )
                return False

            # Validate B/S direction logic for hyphenated products
            if not self._validate_hyphenated_direction_logic(
                exchange_trade, first_trader_trade, second_trader_trade
            ):
            ...
            if (
                exchange_trade.price != first_trader_trade.price
                or exchange_trade.price != second_trader_trade.price
            ):
                logger.debug(
                    f"❌ Price mismatch: exchange={exchange_trade.price}, trader1={first_trader_trade.price}, trader2={second_trader_trade.price}"
                )
                return False
    ...
    def _validate_hyphenated_direction_logic(
        ...
        if (
            first_trader_trade.product_name == first_product
            and second_trader_trade.product_name == second_product
        ):
            first_leg_trade = first_trader_trade
            second_leg_trade = second_trader_trade
        elif (
            first_trader_trade.product_name == second_product
            and second_trader_trade.product_name == first_product
        ):
        ...
        is_valid = (
            actual_first_direction == expected_first_direction
            and actual_second_direction == expected_second_direction
        )

        if not is_valid:
            logger.debug(
                f"❌ Direction logic failed: exchange {exchange_trade.buy_sell} on '{exchange_trade.product_name}' "
