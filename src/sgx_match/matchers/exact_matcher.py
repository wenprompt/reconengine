...
class ExactMatcher(BaseMatcher):
        ...
        for trader_trade in trader_trades:
            ...
            for i in range(len(exchange_trades_list) - 1, -1, -1):
                ...
                if not pool_manager.is_unmatched(
                    exchange_trade.internal_trade_id, SGXTradeSource.EXCHANGE
                ):
                    continue
                ...
                if pool_manager.record_match(match_result):
                    ...
                    logger.warning(
                        f"Failed to atomically record match for {trader_trade.display_id} ↔ {exchange_trade.display_id}"
                    )
    ...
    ) -> dict[tuple[SignatureValue, ...], list[SGXTrade]]:
    ...
    def _create_matching_signature(self, trade: SGXTrade) -> tuple[SignatureValue, ...]:
    ...
    def _create_match_result(
        self, trader_trade: SGXTrade, exchange_trade: SGXTrade
    ) -> SGXMatchResult:
