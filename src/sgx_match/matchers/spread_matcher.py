...
class SpreadMatcher(MultiLegBaseMatcher):
    ...
    def find_matches(self, pool_manager: SGXUnmatchedPool) -> list[SGXMatchResult]:
        ...
        exchange_spread_pairs = self._find_exchange_spread_pairs(
        ...
        for trader_pair in trader_spread_pairs:
            ...
            match_result = self._match_spread_pair(
                trader_pair, exchange_spread_pairs, pool_manager
            )
            if match_result:
                ...
                if pool_manager.record_match(match_result):
                    ...
                    logger.warning(
                        f"Failed to atomically record spread match {match_result.match_id}"
                    )
    ...
    def _is_trader_spread_pair(self, trade1: SGXTrade, trade2: SGXTrade) -> bool:
        ...
        has_spread_indicator = (
            trade1.spread and str(trade1.spread).upper().strip() == "S"
        ) or (trade2.spread and str(trade2.spread).upper().strip() == "S")
    ...
    def _find_exchange_spread_pairs(
        ...
        tier1_pairs = self._find_exchange_spread_pairs_by_dealid(
        ...
        if tier1_pairs:
            logger.info(
                f"✓ TIER 1 Results: Found {tier_counts['tier1']} spread pairs using dealid method"
            )
            ...
            remaining_trades = [
                ...
                if t.internal_trade_id not in tier1_trade_ids
            ]
            logger.debug(
        ...
        if remaining_trades:
            tier2_pairs = self._find_exchange_spread_pairs_by_time(
                remaining_trades, pool_manager
            )
            ...
            if tier2_pairs:
                logger.info(
                    f"✓ TIER 2 Results: Found {tier_counts['tier2']} spread pairs using time-based method"
                )
        ...
        logger.info(
            f"🏁 2-TIER SEQUENTIAL COMPLETE: {total_pairs} spread pairs identified"
        )
        logger.info(
            f"   📊 TIER BREAKDOWN: Tier 1: {tier_counts['tier1']}, Tier 2: {tier_counts['tier2']}"
        )
            ...
            if len(trades_in_group) == 2:
                ...
                if tradeid1 != tradeid2 and tradeid1 and tradeid2:
                    ...
                    if self.validate_spread_pair_characteristics(trade1, trade2):
                        ...
                        logger.debug(
                            f"Found dealid spread pair: {tradeid1}/{tradeid2} (dealid: {dealid_str})"
                        )
        ...
        for datetime_key, trades in time_groups.items():
            ...
            for i in range(len(trades)):
                for j in range(i + 1, len(trades)):
                    ...
                    if not pool_manager.is_unmatched(
                        trade1.internal_trade_id, SGXTradeSource.EXCHANGE
                    ) or not pool_manager.is_unmatched(
                        trade2.internal_trade_id, SGXTradeSource.EXCHANGE
                    ):
                        continue
                    ...
                    if self.validate_spread_pair_characteristics(trade1, trade2):
                        ...
                        spread_price = self._calculate_exchange_spread_price_sgx(
                            trade1, trade2
                        )
    ...
    def _calculate_exchange_spread_price_sgx(
        ...
        try:
            ...
            spread_price = calculate_spread_price(
                trade1.price, trade1.contract_month, trade2.price, trade2.contract_month
            )
    ...
    def _find_matching_trader_spreads_with_price(
        ...
        for i in range(len(trader_trades)):
            for j in range(i + 1, len(trader_trades)):
                ...
                if not pool_manager.is_unmatched(
                    trader1.internal_trade_id, SGXTradeSource.TRADER
                ) or not pool_manager.is_unmatched(
                    trader2.internal_trade_id, SGXTradeSource.TRADER
                ):
                    continue
    ...
    def _validate_trader_spread_matches_exchange(
        ...
        if not self.validate_options_compatibility(
            trader1, exchange1, trader2, exchange2
        ):
            return False
        ...
        if (
            trader1.quantityunit != exchange1.quantityunit
            or trader2.quantityunit != exchange1.quantityunit
        ):
            return False
    ...
    def _validate_spread_prices(
        ...
        if len(set(trader_prices)) != 1:
            logger.debug(
                f"SGX trader spread price validation failed: legs have different prices {trader_prices}"
            )
            return False
        ...
        exchange_spread_price = calculate_spread_price(
            exchange_trade1.price,
            exchange_trade1.contract_month,
            exchange_trade2.price,
            exchange_trade2.contract_month,
        )
