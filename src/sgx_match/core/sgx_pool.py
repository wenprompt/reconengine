...
class SGXUnmatchedPool:
    ...
    def record_match(self, match_result: "SGXMatchResult") -> bool:
        ...
        trades_to_remove.append(
            (match_result.trader_trade.internal_trade_id, SGXTradeSource.TRADER)
        )
        ...
        trades_to_remove.append(
            (match_result.exchange_trade.internal_trade_id, SGXTradeSource.EXCHANGE)
        )
    ...
    def reset(
        ...
        logger.info(
            f"Reset SGX pool with {len(trader_trades)} trader trades and {len(exchange_trades)} exchange trades"
        )
