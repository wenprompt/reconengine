...
class SGXUnmatchedPool:
    """Manages pools of unmatched SGX trades to ensure no duplicate matching.
    ...
    def __init__(self, trader_trades: List[SGXTrade], exchange_trades: List[SGXTrade]):
        ...
        Args:
            trader_trades: List of all SGX trader trades
            exchange_trades: List of all SGX exchange trades
        """
        ...
        logger.info(f"Initialized SGX pool with {len(trader_trades)} trader trades and {len(exchange_trades)} exchange trades")
    ...
    def is_unmatched(self, trade_id: str, source: SGXTradeSource) -> bool:
    ...
    def reset(self, trader_trades: List[SGXTrade], exchange_trades: List[SGXTrade]) -> None:
        ...
        logger.info(f"Reset SGX pool with {len(trader_trades)} trader trades and {len(exchange_trades)} exchange trades")