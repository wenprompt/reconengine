"""Match result data model for CME trade matching system."""
...
class CMEMatchType(str, Enum):
    """Type of matching rule that produced this CME match."""
    EXACT = "exact"  # Rule 1 - Exact field matching for CME trades


class CMEMatchResult(BaseModel):
    """Represents a successful match between CME trades.
    
    Contains information about which trades matched, the rule used,
    confidence level, and audit trail information specific to CME trading.
    """
    ...
    match_type: CMEMatchType = Field(..., description="Type of matching rule applied")
    ...
    # Matched trades
    trader_trade: CMETrade = Field(..., description="The trader trade in this match")
    exchange_trade: CMETrade = Field(..., description="The exchange trade in this match")
    
    # Additional trades for complex matches (future use)
    additional_trader_trades: List[CMETrade] = Field(
        default_factory=list, 
        description="Additional trader trades for multi-leg matches"
    )
    additional_exchange_trades: List[CMETrade] = Field(
        default_factory=list, 
    ...
    def matched_quantity(self) -> Decimal:
        ...
        return self.trader_trade.quantitylot
    ...
    def quantity_difference(self) -> Decimal:
        ...
        return abs(self.trader_trade.quantitylot - self.exchange_trade.quantitylot)
    ...
    def get_all_trades(self) -> List[CMETrade]:
