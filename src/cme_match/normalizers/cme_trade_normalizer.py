"""Trade data normalizer for CME trades."""
...
class CMETradeNormalizer:
    """Normalizes CME trade data from various CSV sources to standardized format."""
    ...
    def __init__(self, config_manager: CMEConfigManager):
        ...
        logger.info("Initialized CME trade normalizer")
    ...
    def normalize_contract_month(self, contract_month: str) -> str:
        ...
        for pattern, replacement in self._month_patterns.items():
            ...
            if match:
                ...
                logger.debug(
                    f"Normalized contract month: '{contract_month}' -> '{normalized}'"
                )
                return normalized

        # If no pattern matches, return cleaned version
        logger.debug(
            f"No pattern match for contract month: '{contract_month}', returning cleaned"
        )
        return cleaned
    ...
    def normalize_buy_sell(self, buy_sell: str) -> str:
        ...
        logger.error(
            f"Unable to normalize buy/sell value: '{buy_sell}' - invalid value"
        )
    ...
    def normalize_quantity(self, quantity: Any) -> Optional[Decimal]:
        ...
        try:
            ...
            logger.error(
                f"Unable to process quantity value '{quantity}': {e}. Check data format and try again."
            )
            return None
    ...
    def normalize_price(self, price: Any) -> Optional[Decimal]:
        ...
        try:
            ...
            logger.error(
                f"Unable to process price value '{price}': {e}. Check data format and try again."
            )
            return None
    ...
    def normalize_integer_field(self, value: Any) -> Optional[int]:
        ...
        try:
            ...
            logger.error(
                f"Unable to process numeric value '{value}': {e}. Check data format and try again."
            )
            return None
