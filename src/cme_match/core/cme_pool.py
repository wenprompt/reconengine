...
class CMEUnmatchedPool:
    ...
    def __init__(self, trader_trades: list[CMETrade], exchange_trades: list[CMETrade]):
        ...
        self._trader_pool: dict[str, CMETrade] = {
            trade.internal_trade_id: trade for trade in trader_trades
        }
        self._exchange_pool: dict[str, CMETrade] = {
        ...
        logger.info(
            f"Initialized CME pool with {len(trader_trades)} trader trades and {len(exchange_trades)} exchange trades"
        )
    ...
    def record_match(self, match_result: "CMEMatchResult") -> bool:
        ...
        trades_to_remove.append(
            (match_result.trader_trade.internal_trade_id, CMETradeSource.TRADER)
        )
        if match_result.additional_trader_trades:
            for trade in match_result.additional_trader_trades:
                trades_to_remove.append(
                    (trade.internal_trade_id, CMETradeSource.TRADER)
                )

        trades_to_remove.append(
            (match_result.exchange_trade.internal_trade_id, CMETradeSource.EXCHANGE)
        )
        if match_result.additional_exchange_trades:
            for trade in match_result.additional_exchange_trades:
                trades_to_remove.append(
                    (trade.internal_trade_id, CMETradeSource.EXCHANGE)
                )
    ...
    def reset(
        ...
        logger.info(
            f"Reset CME pool with {len(trader_trades)} trader trades and {len(exchange_trades)} exchange trades"
        )
