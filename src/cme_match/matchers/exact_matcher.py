...
class ExactMatcher(BaseMatcher):
    ...
    def find_matches(self, pool_manager: CMEUnmatchedPool) -> list[CMEMatchResult]:
        ...
        logger.info(
            f"Processing {len(trader_trades)} trader trades and {len(exchange_trades)} exchange trades"
        )
        ...
        for trader_trade in trader_trades:
            ...
            for i in range(len(exchange_trades_list) - 1, -1, -1):
                ...
                if not pool_manager.is_unmatched(
                    exchange_trade.internal_trade_id, CMETradeSource.EXCHANGE
                ):
                    continue
                ...
                if pool_manager.record_match(match_result):
                    ...
                    logger.debug(
                        f"Created exact match: {trader_trade.display_id} ↔ {exchange_trade.display_id}"
                    )
                    ...
                    logger.warning(
                        f"Could not save match for trades {trader_trade.display_id} ↔ {exchange_trade.display_id}. Trade already matched."
                    )
    ...
    def _create_exchange_index(
        self, exchange_trades: list[CMETrade]
    ) -> dict[tuple[Union[str, int, float, None], ...], list[CMETrade]]:
    ...
    def _create_matching_signature(
        self, trade: CMETrade
    ) -> tuple[Union[str, int, float, None], ...]:
        ...
        # Convert Decimal to float for signature compatibility
        rule_fields: list[SignatureValue] = [
            trade.product_name,
            trade.contract_month,
            float(trade.quantitylot) if trade.quantitylot is not None else None,
    ...
    def _create_match_result(
        self, trader_trade: CMETrade, exchange_trade: CMETrade
    ) -> CMEMatchResult:
