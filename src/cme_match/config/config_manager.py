"""Configuration manager for CME trade matching system."""
...
class CMEMatchingConfig(BaseModel):
    """Configuration for CME trade matching system.

    Contains tolerances, conversion factors, and other configurable parameters
    for the CME matching engine.
    """
    ...
    processing_order: List[int] = Field(
        default=[1],  # Only Rule 1 (exact) for CME
...
class CMEConfigManager:
    """Manages configuration for CME trade matching system.
    ...
    def __init__(self, config_path: Optional[Path] = None):
        ...
        self.matching_config = CMEMatchingConfig()
    ...
    def get_rule_confidence(self, rule_number: int) -> Decimal:
        ...
        Args:
            rule_number: Rule number (1 only for CME)
