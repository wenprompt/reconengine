...
class EEXTrade(BaseModel):
    ...
    broker_group_id: Optional[int] = Field(
        default=None, description="Broker group identifier"
    )
    exch_clearing_acct_id: Optional[int] = Field(
        default=None, description="Exchange clearing account ID"
    )

    # Additional EEX-specific fields
    exchange_group_id: Optional[int] = Field(
        default=None, description="Exchange group identifier"
    )
