...
class BaseMatcher(ABC):
    ...
    def __init__(self, config_manager: EEXConfigManager):
        ...
        logger.debug(f"Initialized {self.__class__.__name__} with universal fields: {self.universal_fields}")
    ...
    def create_universal_signature(self, trade: EEXTrade, 
                                 rule_specific_fields: List) -> Tuple:
    ...
    def get_universal_matched_fields(self, rule_fields: List[str]) -> List[str]:
        ...
        universal_model_fields = [
            self.field_mappings.get(field, field) 
